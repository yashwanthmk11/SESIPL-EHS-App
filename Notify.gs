...
function broadcastGallery_(user, project, title, category) {
  ...
  if (user.role === ROLES.MANAGER || user.role === ROLES.DIRECTOR) {
    ...
    const allUsers = rowsToObjects_(SHEETS.USERS).filter(
...
function notifyHigherManager_(obs, title, body, fromUser) {
  const managers = rowsToObjects_(SHEETS.USERS).filter(
    (u) => u.role === ROLES.MANAGER || u.role === ROLES.DIRECTOR,
  );
  managers.forEach((m) => {
    pushNotify_(
      m.employeeId,
      obs.projectId,
      fromUser || { employeeId: "SYSTEM" },
      title,
      body,
      "ESCALATION",
    );
...
function higherManagerEmails_() {
  return rowsToObjects_(SHEETS.USERS).filter(
    (u) => u.role === ROLES.MANAGER || u.role === ROLES.DIRECTOR,
  );
}
